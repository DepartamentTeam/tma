...
export function UploadFile(props: Props) {
  ...
  const [NoCv, SetNoCv] = createSignal(true);
  ...
  const handleSelectFile = (e: Event) => {
    getBase64(e.target.files[0]).then((b64) => setFile(b64));
  };

  const handleGetInitialCv = async () => {
    // /cv/get_by_telegram_id/{telegramId}
  ...
  const handleUpload = async () => {
    ...
    if (file()) {
      ...
      formData.append("telegramId", store.user.id);
  ...
  const [existedCV] = createResource(handleGetInitialCv);

  createEffect(on(file, handleUploadResponse));

  return (
    <>
      {existedCV()?.response != "CV was not found" && SetNoCv(false)}
      <form noValidate id="form-upload-resume" class={style.container}>
        <button
          type="button"
          onClick={handleClick}
          class={`${NoCv() ? style.noCv : style.Cv}`}
        >
          ...
          {!file() && NoCv() ? (
            <div class={style.label}>
              ...
              <label>
                ...
                <input type="file" onChange={handleSelectFile} accept=".pdf" />
              </label>
              {NoCv() && (
                <Chip theme="error" size="sm" variant="filled" label="no CV" />
              )}
            </div>
          )  : (
            <span>{file()?.name}</span>
